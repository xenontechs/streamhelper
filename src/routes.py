...
@bp.route('/status')
def status():
    return render_template('status.html', statuslist=data.statuslist, navigationData=data.navigationData)
...
def log():
    ...
    return render_template('log.html', navigationData=data.navigationData, logcontents=logcontents)
...
def settings():
    ...
    return render_template('settings.html', config=config, sections=sections, navigationData=data.navigationData)
...
def deck():
    ...
    return render_template('deck.html', nonav=nonav, navigationData=data.navigationData, buttons=data.buttons)
...
def playground():
    ...
    config.read('config.ini')
    nonav = request.args.get('nonav')
    return render_template('playground.html', nonav=nonav, navigationData=data.navigationData, statuslist=data.statuslist, config=config, buttons=data.buttons)
